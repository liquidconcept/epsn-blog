...
module Jekyll
      ...
      if self.layouts.key? 'author_index'
        ...
        self.posts.each do |post|
          ...
          post_authors.each do |author|              
            self.write_author_index(File.join(dir, author.downcase.gsub(/ /, '_')), author)
          end
        end
  ...
  class GenerateAuthor < Generator
    ...
    def generate(site)
      ...
      puts "site.authors #{site.posts.map {|p| [p.data['author']] }.flatten.uniq.sort.join(', ')}"
    end
  ...
  module Filters
    ...
    def author_links(authors)
      ...
      authors = [authors].flatten.map do |author|
        "<a class='author' href='/#{dir}/#{author.downcase.gsub(/ /, '_')}/'>#{author}</a>"
      end
      case authors.length
      when 0
        ""
